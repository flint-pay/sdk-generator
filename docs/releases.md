...
For added response statuses, objects that combine named `properties` with schema-valued `additionalProperties` still require manual review of their additional-field runtime guarantees. The comparison treats their additional values as `unknown`, matching TypeScript declarations, and can miss nested required-key removal. For example, if a response declares a `label` property and requires `id` inside every additional object value, adding a status that permits those values without `id` can break consumers while receiving only a `review` finding. Under `semver`, that finding alone permits a patch release; choose a major release for the lost guarantee. Pure dictionaries without named properties retain their runtime-requiredness checks.
