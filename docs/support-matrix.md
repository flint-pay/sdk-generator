...
| Schemas        | Objects, arrays, scalars, exact numbers, recursive models, additionalProperties; const; allOf/anyOf/oneOf/not/contains/if/then/else; dependentRequired; 3.1 schema `$ref` siblings                                                      | Unlisted validation keywords, including contains count modifiers                                                                                |
...
| Constraints    | Explicit schema-validation profile for request bounds, Unicode lengths/patterns, array lengths, uniqueItems, multipleOf, contains and object property counts; integer ranges always checked; tolerant business constraints on responses | Arbitrary regex extensions, simple nested unbounded repetition, and unlisted JSON Schema validation keywords                                    |
...
`dependentRequired` preserves property dependencies through equivalent conditional constraints. A present trigger requires every listed property; absent triggers impose no requirement. Dependency maps must contain arrays of unique property names.
