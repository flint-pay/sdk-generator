...
Unknown configuration keys fail with a source location. Capability declarations belong under the relevant `operations` entry unless marked as top-level below.
...
See [CLI usage](cli.md) to diagnose or preview these inputs, and [using generated SDKs](using-sdks.md) for runtime client/request options. Credentials and runtime base URLs are supplied by consumers, not stored in this file.
...
- `money` (top level): `{ "currencies": { "USD": 2, "JPY": 0 } }`. Conversion accepts an exact major-unit decimal string and produces an exact minor-unit string. Excess precision is rejected; implicit rounding and guessed currency tables are absent.
- `webhook` (top level): see below. Other signing protocols fail with actionable diagnostics.
...
These checks invoke generated public clients in the selected targets, including their input and response models. Independent review of provider expectations and actual sandbox tests remain separate responsibilities; fixture execution does not perform either review or sandbox acceptance.
...
Named model factories can be nested in TypeScript client inputs through `InputValue<T>`. Factories normalize nested wrappers into plain typed values, and request serialization revalidates them. PHP wrappers preserve object/array/scalar/null shapes; `toArray()` applies only to array or object models. Cyclic values, including opaque additional fields, fail before HTTP dispatch.

Scalar alternatives mixing strings with exact-number strings, or numeric intersections requiring different SDK representations, currently receive source diagnostics. Safe-integer/string alternatives remain distinct and supported. Use a provider correction only when it preserves the upstream wire contract; do not silently rewrite ambiguous values.
