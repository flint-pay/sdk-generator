...
function callPlan(op: Operation, plan: CompiledSdkContract, optionsType: string) {
  ...
  const required = plan.node.operations[op.id]!.inputRequired;
  if (!positional(op))
    return {
...
function exampleSchema(op: Operation, apiVersion?: Contract['config']['apiVersion']): Schema {
  const schema = inputSchema(op, apiVersion);
  // Examples have no separate ifMatch option, so retain a required precondition
  // in their canonical input even though public inputs can use request options.
  for (const p of op.parameters)
    if (
      p.required &&
      p.in === 'header' &&
      p.name.toLowerCase() === op.conditional?.header.toLowerCase() &&
      p.name.toLowerCase() !== apiVersion?.header.toLowerCase()
    )
...
function exampleInput(op: Operation, definitions: Record<string, Schema> = {}): unknown {
  ...
  try {
    const input = op.example ?? materializeSample(sample(exampleSchema(op), {}, definitions));
...
function renderCompiled(compilation: ReturnType<typeof compileSdkContract>): Map<string, string> {
  ...
  for (const op of c.operations) {
    ...
    try {
      const schema = exampleSchema(op, c.config.apiVersion);
      const input = op.example ?? materializeSample(sample(schema, {}, c.definitions ?? {}));
      for (const key of schema.required ?? [])
        if (!input || typeof input !== 'object' || !Object.hasOwn(input, key))
          throw new Error(`required example input ${key} is missing`);
  ...
  if (targets.includes('php')) {
    ...
    for (const group of groups) {
      ...
      for (const op of c.operations.filter((o) => o.resource === group)) {
        ...
        const inputDefault = inputSchema(op, c.config.apiVersion).required!.length ? '' : ' = []';
  ...
  for (const target of targets) {
    ...
    const guidance =
      ...
      '\n\n';
    const resultData = target === 'node' ? 'result.data' : '$result->data';
    const resultMeta = target === 'node' ? 'result.meta' : '$result->meta';
    const nestedData = target === 'node' ? 'result.data.data' : '$result->data->data';
    const responseGuidance = hasPayloadReturns
      ? `Payload-mode calls return the decoded payload directly. Use the corresponding WithResponse method for the complete body and HTTP metadata; see the [WithResponse example](RUNTIME.md#response-return-modes). Each invocation makes its own request; choose one form per action. Operations configured for result mode retain \`${resultData}\` and \`${resultMeta}\`. The API reference describes the return mode for each operation.`
        ...
        [...c.operations]
          ...
          .map(
            (op, index, operations) =>
              ...
              }\`\n\n${fieldTable(inputSchema(op, c.config.apiVersion), c.definitions ?? c.models)}${operationGuidance(c, op)}${op.idempotency ? `Idempotency header: ${op.idempotency.header}; retention: ${op.idempotency.retention}; scope: ${op.idempotency.scope}.\n\n` : ''}${op.aliases?.length ? `Deprecated aliases: ${op.aliases.join(', ')} (same wire operation).\n\n` : ''}[Example](examples/${op.resource}-${op.method}.${target === 'node' ? 'mjs' : 'php'})\n\n${convenienceReference(c, op, target)}`,
          )
          .join('\n'),
    );
  }
...
export function preview(contract: Contract, output: string) {
  ...
  if (before && contract.config.responses?.return === undefined) {
    const previous = new Map(before.interface.operations.map((op) => [op.id, op]));
    for (const op of contract.operations)
      if (
        previous.has(op.id) &&
        previous.get(op.id)!.response?.return !== 'payload' &&
        op.response?.return === 'payload' &&
        contract.config.operations?.[op.id]?.response?.return === undefined
      )
        throw new Diagnostic(
          'config/responses/return',
